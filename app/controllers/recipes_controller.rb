class RecipesController < ApplicationController
  ...
  def index
    if params[:search].present?
      @recipes = Recipe.where("name ILIKE ?", "%#{params[:search]}%").order(:name)
    else
      @recipes = Recipe.order(:name)
    end
    return render partial: "recipes_results", locals: { recipes: @recipes } if turbo_frame_request?
  end
  ...
  private
    ...
    def recipe_params
      params.expect(recipe: [ :name, :description, :directions ])
    end
end
